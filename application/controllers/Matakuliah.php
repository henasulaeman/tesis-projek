...
class Matakuliah extends CI_Controller {
	...
	public function proses_insert_matkul(){
		...
		$this->Matakuliah_model->insert('tbl_matakuliah', $data_matkul);
		redirect('Matakuliah');
	...
	public function form_edit_matkul(){
		...
		$data['dt_matkul'] = $this->Matakuliah_model->edit($id);
		//$data['dt_prodi'] = $this->Prodi_model->show('tbl_program_studi');
	...
	public function proses_update_matkul(){
		...
		$this->Matakuliah_model->update($id,$data_matkul);

		redirect('Matakuliah');

	}
	public function proses_delete_matkul(){
		$id = $this->uri->segment(3);
